...
@st.cache_resource
def cargar_modelo():
    return joblib.load("../models/modelo_final.pkl")
...
with col2:
    cred         = st.selectbox("¿Tiene CRED al día?", options=[1, 0],
                                format_func=lambda x: "Sí" if x == 1 else "No")
    suplemento   = st.selectbox("¿Recibe suplemento de hierro?", options=[1, 0],
                                format_func=lambda x: "Sí" if x == 1 else "No")
    juntos       = st.selectbox("¿Pertenece al programa Juntos?", options=[0, 1],
                                format_func=lambda x: "Sí" if x == 1 else "No")
    sis          = st.selectbox("¿Tiene SIS?", options=[1, 0],
                                format_func=lambda x: "Sí" if x == 1 else "No")
    qaliwarma    = st.selectbox("¿Pertenece a Qali Warma?", options=[0, 1],
                                format_func=lambda x: "Sí" if x == 1 else "No")
...
if st.button("🔍 Predecir diagnóstico", use_container_width=True, type="primary"):
    ...
    if etiqueta != "Normal":
        st.warning(
            "⚠️ Este resultado es una predicción orientativa. "
...
st.divider()
st.caption(
    "Modelo: Random Forest | Dataset: SIEN Ayacucho | "
    "Proyecto Integrador — Ciencia de Datos"
)
